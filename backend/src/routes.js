...
routes.get('/dashboard', DashboardController.show); // rota para ver seus spots ou criar apos fazer login

routes.post('/spots/:spot_id/bookings', BookingController.store); // rota para fazer reserva em um spot escolhido

routes.post('/bookings/:booking_id/approvals', ApprovalController.store); // rota para enviar solicitaçã de reserva Aprovada
routes.post('/bookings/:booking_id/rejections', RejectionController.store); // rota para enviar solicitaçã de reserva Rejeitada
